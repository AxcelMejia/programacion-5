...
void Imprime(int VEC[], int T);    /* Observa que en los parámetros, para indicar que lo que se recibe es un arreglo, se puede escribir VEC[]o **VEC. */
...
void main(void)
{
    ...
    Lectura(VE1, MAX);           /* Se llama a la función LLeeccttuurraa. Observa que el paso del arreglo a la función es por referencia. Sólo se debe incluir el nombre del arreglo. */
    Lectura(VE2, MAX);
...
/* La función LLeeccttuurraa se utiliza para leer un arreglo unidimensional de T elementos de tipo entero. */
{
    int I;
    printf("\n");
    for (I=0; I<T; I++)
    ...
    /* La función IImmpprriimmee se utiliza para imprimir un arreglo unidimensional de T elementos de tipo entero. */
    int I;
    {
        for (I=0; I<T; I++)
            printf("\nVEC[%d]: %d", I+1, VEC[I]);
